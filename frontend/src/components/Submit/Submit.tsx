...
export default function Submit(props: SubmitProps) {
    ...
    const getFileList = () => { 
        ...
        if (props.coordinateFile) {
            for(const item in props.coordinateFile){ 
                array.push(<FileLine icon={<Molecule/>} name={props.coordinateFile[item].name} key={`c${item}`}/>)
            }
        }
    ...
    return (
        <div
            className="flex flex-col m-2 px-2 pt-8 w-64 
