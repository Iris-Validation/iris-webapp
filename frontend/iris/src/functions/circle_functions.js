export function calculate_poly_line(center, radius, data) {
    const header = 40;
...
export function calculate_poly_line_for_circle(center, radius) {
    const header = 40;
...
export function calculate_center_line(center, angle, radius) {
    angle = angle * (Math.PI / 180);
    const x = center[0] + radius * Math.sin(angle);
    const y = center[1] - radius * Math.cos(angle);
    return `${center[0]},${center[1]} ${x},${y}`;
}
export function calculate_text_position(center, text, radius) {
