...
export default function NavBar({setResetApp}: {setResetApp: Dispatch<SetStateAction<boolean>>}) {
    ...
    return (
        ...
        <div className="text-center sm:text-left px-12 pt-12 sm:p-12 flex flex-col">
            <span
                className="font-primary text-xl text-secondary sm:text-3xl">Validate your proteins online with</span>
            <span className="font-body text-4xl sm:text-6xl my-2 sm:my-1"><button id="title" title="Home"
                                                                                  className="bg-gray"
                                                                                  onClick={() => {setResetApp(true)}}>Iris</button></span>
            <span className="font-primary text-l text-secondary sm:text-xl sm:my-4 my-2">The interactive all-in-one graphical validation of 3D protein model web app</span>
        </div>
